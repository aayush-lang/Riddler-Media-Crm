...
let state = {
  ...
  reminders: [], config: { services: [], sources: [], stages: [] },
...
const STAGE_COLORS = { Lead: '#3B82F6', Contacted: '#6366F1', Pitched: '#F59E0B', Negotiation: '#10B981', Won: '#22C55E', Lost: '#EF4444' };
...
async function loadConfig() { const { data } = await db.from('config').select('*'); if (data) { data.forEach(row => { state.config[row.key] = row.value; }); } populateSelects(); }
...
function populateSelects() {
  const svcs = state.config.services || []; const srcs = state.config.sources || [];
  const fsvc = document.getElementById('f-service'); const fsrc = document.getElementById('f-source');
  if (fsvc) fsvc.innerHTML = '<option value="">All services</option>' + svcs.map(s => `<option>${s}</option>`).join('');
  if (fsrc) fsrc.innerHTML = '<option value="">All sources</option>' + srcs.map(s => `<option>${s}</option>`).join('');
  const lfsvc = document.getElementById('lf-service'); const lfsrc = document.getElementById('lf-source');
  if (lfsvc) lfsvc.innerHTML = '<option value=""></option>' + svcs.map(s => `<option>${s}</option>`).join('');
  if (lfsrc) lfsrc.innerHTML = '<option value=""></option>' + srcs.map(s => `<option>${s}</option>`).join('');
  ...
  if (sol) sol.innerHTML = srcs.map(s => `<span class="config-tag">${s}</span>`).join('');
}
...
function renderDashboard() {
  const leads = state.leads; const won = leads.filter(l => l.stage === 'Won');
  ...
  document.getElementById('metrics-row').innerHTML = `
    ...
    <div class="metric-card"><div class="metric-label">Conversion rate</div><div class="metric-value green">${conv}%</div><div class="metric-sub">${won.length} deals won</div></div>
  ...
  const stages = ['Lead', 'Contacted', 'Pitched', 'Negotiation', 'Won', 'Lost'];
  const max = Math.max(...stages.map(s => leads.filter(l => l.stage === s).length), 1);
  document.getElementById('stage-bars').innerHTML = stages.map(s => { const c = leads.filter(l => l.stage === s).length; return `<div class="stage-bar-row"><span class="stage-bar-label">${s}</span><div class="stage-bar-track"><div class="stage-bar-fill" style="width:${Math.round(c/max*100)}%;background:${STAGE_COLORS[s]}"></div></div><span class="stage-bar-count">${c}</span></div>`; }).join('');
  const srcMap = {}; leads.forEach(l => { if (l.source) srcMap[l.source] = (srcMap[l.source] || 0) + 1; });
  const srcEntries = Object.entries(srcMap).sort((a, b) => b[1] - a[1]).slice(0, 6);
  document.getElementById('source-chart').innerHTML = srcEntries.length ? srcEntries.map(([s, c]) => `<div class="source-row"><span>${s}</span><span class="source-pill">${c}</span></div>`).join('') : '<div class="empty-state">No source data yet</div>';
  ...
  const perfMap = {}; leads.forEach(l => { if (!l.assigned_to) return; const prof = state.profiles.find(p => p.id === l.assigned_to); const name = prof?.name || 'Unknown'; if (!perfMap[name]) perfMap[name] = { total: 0, won: 0 }; perfMap[name].total++; if (l.stage === 'Won') perfMap[name].won++; });
  const perfEntries = Object.entries(perfMap).sort((a, b) => b[1].total - a[1].total);
  document.getElementById('team-perf').innerHTML = perfEntries.length ? perfEntries.map(([name, p]) => `<div class="team-row"><span style="font-weight:500">${name}</span><div class="team-stats"><div class="team-stat"><div class="team-stat-num">${p.total}</div><div class="team-stat-lbl">Leads</div></div><div class="team-stat"><div class="team-stat-num">${p.won}</div><div class="team-stat-lbl">Won</div></div><div class="team-stat"><div class="team-stat-num">${p.total ? Math.round(p.won/p.total*100) : 0}%</div><div class="team-stat-lbl">Conv.</div></div></div></div>`).join('') : '<div class="empty-state">Assign leads to team members to see stats</div>';
}
...
function handleSort(col) {
  ...
  document.querySelectorAll('th.sortable').forEach(th => { th.classList.remove('sort-asc', 'sort-desc'); if (th.dataset.col === col) th.classList.add(state.sortDir === 'asc' ? 'sort-asc' : 'sort-desc'); });
...
function renderLeads() {
  ...
  if (!slice.length) { tbody.innerHTML = `<tr><td colspan="10" class="empty-row"><div class="empty-state-icon">🔍</div><div>No leads found</div></td></tr>`; }
  else {
    tbody.innerHTML = slice.map(l => {
      ...
      return `<tr data-id="${l.id}" class="${state.selectedLeads.has(l.id) ? 'selected' : ''}">
        ...
        <td><span class="stage-badge stage-${l.stage}">${l.stage}</span></td>
...
function openAddLead() {
  state.editLeadId = null; document.getElementById('lead-modal-title').textContent = 'Add new lead'; document.getElementById('edit-lead-id').value = '';
  ...
  document.getElementById('lf-stage').value = 'Lead'; document.getElementById('lf-type').value = 'Prospect'; document.getElementById('lf-service').value = ''; document.getElementById('lf-source').value = ''; document.getElementById('lf-assigned').value = '';
  openModal('add-lead-modal');
}

function openEditLead(id) {
  const l = state.leads.find(x => x.id === id); if (!l) return;
  state.editLeadId = id; document.getElementById('lead-modal-title').textContent = 'Edit lead'; document.getElementById('edit-lead-id').value = id;
  document.getElementById('lf-name').value = l.name || ''; document.getElementById('lf-company').value = l.company || ''; document.getElementById('lf-email').value = l.email || ''; document.getElementById('lf-phone').value = l.phone || '';
  document.getElementById('lf-stage').value = l.stage || 'Lead'; document.getElementById('lf-type').value = l.type || 'Prospect'; document.getElementById('lf-service').value = l.service || ''; document.getElementById('lf-source').value = l.source || '';
  document.getElementById('lf-value').value = l.value || ''; document.getElementById('lf-city').value = l.city || ''; document.getElementById('lf-notes').value = l.notes || ''; document.getElementById('lf-followup').value = l.followup_date || ''; document.getElementById('lf-assigned').value = l.assigned_to || '';
  openModal('add-lead-modal');
}
...
async function openLeadDetail(id) {
  ...
  document.getElementById('lead-detail-panel').innerHTML = `
    <div class="panel-header"><div><div style="font-size:17px;font-weight:600">${esc(l.name)}</div><div style="font-size:13px;color:var(--text-3)">${esc(l.company || '')}</div><div style="margin-top:8px"><span class="stage-badge stage-${l.stage}">${l.stage}</span> <span class="type-badge type-${l.type}">${l.type}</span></div></div><button class="modal-close" onclick="document.getElementById('lead-detail-overlay').style.display='none'">✕</button></div>
    <div class="panel-section"><div class="panel-section-title">Contact details</div><div class="info-grid"><div class="info-field"><div class="info-label">Email</div><div class="info-value">${esc(l.email || '—')}</div></div><div class="info-field"><div class="info-label">Phone</div><div class="info-value">${esc(l.phone || '—')}</div></div><div class="info-field"><div class="info-label">City</div><div class="info-value">${esc(l.city || '—')}</div></div><div class="info-field"><div class="info-label">Service</div><div class="info-value">${esc(l.service || '—')}</div></div><div class="info-field"><div class="info-label">Source</div><div class="info-value">${esc(l.source || '—')}</div></div><div class="info-field"><div class="info-label">Deal value</div><div class="info-value" style="font-family:'DM Mono',monospace;color:var(--purple)">₹${(+l.value||0).toLocaleString('en-IN')}</div></div><div class="info-field"><div class="info-label">Follow-up</div><div class="info-value">${l.followup_date ? formatDate(l.followup_date) : '—'}</div></div><div class="info-field"><div class="info-label">Assigned to</div><div class="info-value">${prof ? prof.name : '—'}</div></div></div>${l.notes ? `<div style="margin-top:10px;font-size:13px;color:var(--text-2);background:var(--surface-2);padding:10px;border-radius:var(--radius-sm)">${esc(l.notes)}</div>` : ''}</div>
    <div class="panel-section"><div class="panel-section-title">Move stage</div><div class="stage-switcher">${['Lead','Contacted','Pitched','Negotiation','Won','Lost'].map(s => `<button class="stage-switch-btn ${l.stage === s ? 'active' : ''}" onclick="changeStageFromPanel('${l.id}','${s}')">${s}</button>`).join('')}</div></div>
    <div class="panel-section"><div class="panel-section-title">Quick actions</div><div style="display:flex;gap:8px;flex-wrap:wrap"><button class="btn-sm" onclick="openEditLead('${l.id}');document.getElementById('lead-detail-overlay').style.display='none'">Edit lead</button><button class="btn-sm" onclick="openReminderForLead('${l.id}')">+ Reminder</button><button class="btn-danger-sm" onclick="deleteLead('${l.id}')">Delete</button></div></div>
    <div class="panel-section"><div class="panel-section-title">Activity & comments</div><div class="activity-list" id="activity-list-${id}">${(acts || []).map(a => `<div class="activity-item"><div class="activity-dot ${a.type}"></div><div class="activity-content"><div class="activity-text">${a.type === 'comment' ? '💬 ' : ''}${esc(a.text)}</div><div class="activity-author">${a.user?.name || 'System'} · ${formatDateTime(a.created_at)}</div></div></div>`).join('') || '<div style="font-size:13px;color:var(--text-3)">No activity yet</div>'}</div><div class="comment-composer"><textarea class="comment-input" id="comment-input-${id}" rows="2" placeholder="Add a comment or note…"></textarea><button class="btn-primary" style="align-self:flex-end" onclick="postComment('${l.id}')">Post</button></div></div>`;
...
function renderKanban() {
  ...
  document.getElementById('kanban-board').innerHTML = stages.map(stage => {
    ...
    return `<div class="kanban-col" data-stage="${stage}" ondragover="kanbanDragOver(event,this)" ondrop="kanbanDrop(event,'${stage}')" ondragleave="kanbanDragLeave(this)"><div class="col-header"><div class="col-title-wrap"><div class="col-accent" style="background:${STAGE_COLORS[stage]}"></div><span class="col-name">${stage}</span></div><span class="col-count">${cards.length}</span></div><div class="col-cards" id="col-${stage}">${cards.map(l => `<div class="kanban-card" draggable="true" data-id="${l.id}" ondragstart="kanbanDragStart(event,'${l.id}')" ondragend="kanbanDragEnd(event)" onclick="openLeadDetail('${l.id}')"><div class="kcard-name">${esc(l.name)}</div><div class="kcard-company">${esc(l.company || '—')}</div><div class="kcard-footer"><span class="kcard-value">${l.value ? '₹' + (+l.value).toLocaleString('en-IN') : ''}</span><span class="kcard-service">${esc(l.service || '')}</span></div></div>`).join('')}</div></div>`;
  }).join('');
}
...
async function sendReminderEmail(reminder, lead) { if (!RESEND_API_KEY || RESEND_API_KEY === 'YOUR_RESEND_API_KEY') return; const assignee = state.profiles.find(p => p.id === reminder.assigned_to); if (!assignee?.email) return; const body = `<div style="font-family:sans-serif;max-width:520px;margin:0 auto;padding:24px"><div style="background:#4F46E5;color:white;padding:16px 24px;border-radius:8px 8px 0 0"><strong>Riddler CRM</strong> · Reminder</div><div style="background:#f8f7ff;padding:24px;border-radius:0 0 8px 8px;border:1px solid #e5e7eb"><h2 style="margin:0 0 12px;color:#1e1b4b">${reminder.title}</h2>${lead ? `<p style="color:#4b5563"><strong>Lead:</strong> ${lead.name}${lead.company ? ` (${lead.company})` : ''}</p>` : ''}${reminder.notes ? `<p style="color:#4b5563"><strong>Notes:</strong> ${reminder.notes}</p>` : ''}<p style="color:#9ca3af;font-size:12px;margin-top:16px">Due: ${formatDate(reminder.due_date)} at ${reminder.due_time}</p><a href="https://crm.riddlermedia.in" style="display:inline-block;margin-top:16px;background:#4F46E5;color:white;padding:10px 20px;border-radius:6px;text-decoration:none;font-size:13px">Open CRM</a></div></div>`; await fetch('https://api.resend.com/emails', { method: 'POST', headers: { 'Authorization': `Bearer ${RESEND_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ from: FROM_EMAIL, to: [assignee.email], subject: `🔔 Reminder: ${reminder.title}`, html: body }) }); }

function exportCSV() { const headers = ['Name','Company','Email','Phone','Stage','Type','Service','Source','Value','City','Follow-up Date','Notes']; const rows = state.leads.map(l => [l.name,l.company,l.email,l.phone,l.stage,l.type,l.service,l.source,l.value,l.city,l.followup_date,l.notes].map(v => `"${(v||'').toString().replace(/"/g,'""')}"`).join(',')); const csv = [headers.join(','), ...rows].join('\n'); const a = document.createElement('a'); a.href = 'data:text/csv;charset=utf-8,' + encodeURIComponent(csv); a.download = `riddler_leads_${new Date().toISOString().split('T')[0]}.csv`; a.click(); }

function importCSV(event) { const file = event.target.files[0]; if (!file) return; const reader = new FileReader(); reader.onload = async e => { const lines = e.target.result.split('\n').filter(Boolean); const headers = lines[0].split(',').map(h => h.replace(/"/g,'').toLowerCase().trim()); const fieldMap = { name: ['name','full name','contact name'], company: ['company','business','company name'], email: ['email','email address'], phone: ['phone','mobile','contact number'], stage: ['stage'], type: ['type'], service: ['service','service interest'], source: ['source','lead source'], value: ['value','deal value','amount'], city: ['city','location'] }; const colIndex = {}; Object.entries(fieldMap).forEach(([key, aliases]) => { const idx = headers.findIndex(h => aliases.includes(h)); if (idx !== -1) colIndex[key] = idx; }); const rows = lines.slice(1).map(line => { const cells = line.match(/(\".*?\"|[^,]+)/g)?.map(c => c.replace(/^"|"$/g,'').trim()) || []; return cells; }); const toInsert = rows.filter(r => r.length > 1 && r[colIndex.name || 0]).map(r => ({ name: r[colIndex.name] || 'Unknown', company: r[colIndex.company] || '', email: r[colIndex.email] || '', phone: r[colIndex.phone] || '', stage: r[colIndex.stage] || 'Lead', type: r[colIndex.type] || 'Prospect', service: r[colIndex.service] || '', source: r[colIndex.source] || '', value: +(r[colIndex.value] || 0) || 0, city: r[colIndex.city] || '', created_by: state.user.id })); if (!toInsert.length) { alert('No valid rows found in CSV.'); return; } if (!confirm(`Import ${toInsert.length} leads?`)) return; for (let i = 0; i < toInsert.length; i += 500) { await db.from('leads').insert(toInsert.slice(i, i + 500)); } await loadLeads(); renderLeads(); renderDashboard(); alert(`✓ Imported ${toInsert.length} leads successfully!`); }; reader.readAsText(file); event.target.value = ''; }

async function inviteTeamMember() { const email = document.getElementById('invite-email').value.trim(); if (!email) return; alert(`Send this link to ${email}: https://crm.riddlermedia.in\n\nCreate their account from Supabase dashboard → Auth → Users → Invite user.`); document.getElementById('invite-email').value = ''; }
