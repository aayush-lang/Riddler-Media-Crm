...
async function openLeadDetail(id){
  ...
  document.getElementById('lead-detail-panel').innerHTML=`
    ...
    <div class="panel-section">
      ...
      <div class="activity-list" id="activity-list-${id}">
...
async function assignLead(leadId){
  ...
  await db.from('leads').update({assigned_to:newOwner||null,updated_at:new Date().toISOString()}).eq('id',leadId);
  await db.from('activities').insert({lead_id:leadId,user_id:state.user.id,type:'edit',text:`Lead assigned to ${ownerName}`});
...
async function changeStageFromPanel(leadId,stage){
  ...
  await loadLeads();renderLeads();renderDashboard();if(state.activeView==='pipeline')renderKanban();openLeadDetail(leadId);
}
...
<parameter name="path">/home/claude/riddler-crm/app-v2.js</parameter>
