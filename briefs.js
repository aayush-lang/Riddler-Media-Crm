...
export function initBriefs(db, state, esc, formatDate, isAdmin, visibleBriefs) {
  ...
  function renderBriefs() {
    ...
    list.innerHTML = briefs.map(b => {
      ...
      // Show owner chip for admin
      const ownerProfile = isAdmin() ? state.profiles?.find(p => p.id === b.created_by) : null;
      const ownerChip = ownerProfile
        ? '<span class="brief-tag" style="background:var(--blue-light);color:var(--blue)">' + esc(ownerProfile.name) + '</span>'
        : '';
      return '<div class="brief-card">'
        ...
        + (isAdmin() ? '<button class="btn-danger-sm" onclick="deleteBrief(\'' + b.id + '\')">Delete</button>' : '')
        + '</div>'
        + '</div>';
    }).join('');
  }
  ...
  function buildBriefForm(brief) {
    ...
    return '<input type="hidden" id="bf-id" value="' + (v.id||'') + '" />'
      + '<input type="hidden" id="bf-lead-id" value="' + (v.lead_id||'') + '" />'
      + '<input type="hidden" id="bf-lead-name" value="' + esc(v.lead_name||'') + '" />'

      + '<div class="brief-section"><div class="brief-section-title">Basic Information</div><div class="brief-form-grid">'
      ...
      + '<div class="form-field"><label>Sales POC</label>' + radioGroup('sales_poc', ['Shivangi', 'Aayush'], v.sales_poc) + '</div>'
      + '</div></div>'

      + '<div class="brief-section"><div class="brief-section-title">Requirement Type</div>'
      + '<div class="form-field">' + radioGroup('requirement_type', ['Influencer Marketing', '360° Marketing', 'Performance Marketing', 'Social Media Management', 'SEO / Website'], v.requirement_type) + '</div>'
      + '</div>'

      + '<div class="brief-section"><div class="brief-section-title">Influencer Marketing</div><div class="brief-form-grid">'
      ...
      + '<div class="brief-section"><div class="brief-section-title">360° Marketing</div><div class="brief-form-grid">'
      ...
      + '<div class="brief-section"><div class="brief-section-title">Performance Marketing</div><div class="brief-form-grid">'
      ...
      + '<div class="brief-section"><div class="brief-section-title">Client Assets & Budget</div><div class="brief-form-grid">'
      ...
      + '<div class="brief-section"><div class="brief-section-title">SEO / Website</div><div class="brief-form-grid">'
  ...
  window.openBriefModal = async function(briefIdOrLeadId, isLead = false) {
    ...
    if (briefIdOrLeadId && !isLead) {
      ...
      // Non-admin can only edit their own briefs
      if (!isAdmin() && existing && existing.created_by !== state.user.id) {
        alert('You can only edit your own briefs.');
        return;
      }
  ...
  window.viewBrief = function(id) {
    ...
    if (!isAdmin() && b.created_by !== state.user.id) {
      alert('You can only view your own briefs.');
      return;
    }
    ...
    function row(label, value) {
      ...
      const display = Array.isArray(value)
        ? '<div class="brief-tags-display">' + value.map(v => '<span class="brief-tag">' + esc(v) + '</span>').join('') + '</div>'
        : esc(value);
    ...
    document.getElementById('brief-view-panel').innerHTML =
      ...
      + '<div style="font-size:13px;color:var(--text-3)">' + (b.brief_date ? formatDate(b.brief_date) : '')
      + (ownerProfile ? ' · Created by ' + esc(ownerProfile.name) : '') + '</div>'
      + '</div>'
      + '<button class="modal-close" onclick="document.getElementById(\'brief-view-overlay\').style.display=\'none\'">✕</button>'
      + '</div>'

      + '<div class="brief-view-section"><div class="brief-view-title">Basic Information</div>'
      ...
      + ((b.platforms?.length || b.influencer_tier?.length || b.num_influencers) ?
        '<div class="brief-view-section"><div class="brief-view-title">Influencer Marketing</div>'
        + row('Platforms', b.platforms) + row('Influencer Tier', b.influencer_tier)
        + row('No. of Influencers', b.num_influencers) + row('Content Type', b.content_type)
        + row('Genre / Category', b.genre) + row('Deliverables', b.deliverables)
        + row('Campaign Timeline', b.campaign_timeline) + row('Content Format', b.content_format)
        + row('Client Brief', b.client_brief) + '</div>' : '')

      + ((b.business_objective?.length || b.channels_required?.length) ?
        '<div class="brief-view-section"><div class="brief-view-title">360° Marketing</div>'
        + row('Business Objective', b.business_objective) + row('Channels Required', b.channels_required)
        + row('Campaign Timeline', b.timeline_360) + row('Expected Deliverables', b.deliverables_360)
        + row('Client Brief', b.brief_360) + '</div>' : '')

      + ((b.primary_goal?.length || b.platforms_perf?.length) ?
        '<div class="brief-view-section"><div class="brief-view-title">Performance Marketing</div>'
        + row('Primary Goal', b.primary_goal) + row('Platforms', b.platforms_perf)
        + row('Monthly Ad Spend', b.ad_spend_budget) + row('Pixel Status', b.pixel_status)
        + row('Timeline', b.timeline_perf) + row('Client Brief', b.brief_perf) + '</div>' : '')

      + ((b.client_website || b.total_budget) ?
        '<div class="brief-view-section"><div class="brief-view-title">Client Assets & Budget</div>'
        + row('Client Website', b.client_website) + row('Social Handles', b.social_handles)
        + row('Product / App Link', b.product_link) + row('Reference Campaigns', b.reference_campaigns)
        + row('Total Budget', b.total_budget) + row('Budget Allocation', b.budget_allocation) + '</div>' : '')

      + ((b.website_platform || b.target_keywords) ?
        '<div class="brief-view-section"><div class="brief-view-title">SEO / Website</div>'
        + row('Website Platform', b.website_platform) + row('Primary Objective', b.primary_objective)
        + row('Target Keywords', b.target_keywords) + row('Competitors', b.competitors)
        + row('Monthly Traffic', b.monthly_traffic) + row('SEO Scope', b.seo_scope)
        + row('Blog Strategy', b.blog_strategy) + row('Website Optimization', b.website_optimization)
        + row('Timeline', b.timeline_seo) + '</div>' : '')

      + '<div class="brief-view-section"><div style="display:flex;gap:8px;flex-wrap:wrap">'
  ...
  window.deleteBrief = async function(id) {
    if (!isAdmin()) { alert('Only admins can delete briefs.'); return; }
  ...
  function buildPublicBriefView(b) {
    ...
    return '<!DOCTYPE html><html><head><meta charset="UTF-8"><meta name="viewport" content="width=device-width,initial-scale=1">'
      ...
      + '<style>body{font-family:-apple-system,BlinkMacSystemFont,"Segoe UI",sans-serif;margin:0;padding:20px;background:#f8f7ff;color:#1e1b4b}.container{max-width:720px;margin:0 auto;background:white;border-radius:12px;overflow:hidden;box-shadow:0 4px 20px rgba(79,70,229,0.1)}.header{background:#4F46E5;color:white;padding:24px 32px}.header h1{margin:0 0 4px;font-size:22px}.header p{margin:0;opacity:0.8;font-size:14px}.section{padding:20px 32px;border-bottom:1px solid #f0f0f0}.section h2{font-size:12px;font-weight:700;text-transform:uppercase;letter-spacing:1px;color:#4F46E5;margin:0 0 12px}table{width:100%;border-collapse:collapse;font-size:14px}.footer{padding:16px 32px;background:#f8f7ff;font-size:12px;color:#9ca3af;text-align:center}@media print{body{background:white}.footer{display:none}}</style>'
      + '</head><body><div class="container">'
      + '<div class="header"><h1>' + (b.brand_name || 'Marketing Brief') + '</h1><p>Riddler Media · ' + (b.brief_date || '') + ' · ' + (b.requirement_type || '') + '</p></div>'
      + '<div class="section"><h2>Basic Information</h2><table>'
      + row('Brand / Agency', b.brand_name) + row('Industry', b.industry)
      + row('Client POC', b.client_poc_name) + row('POC Contact', b.client_poc_contact)
      + row('Sales POC', b.sales_poc) + row('Requirement Type', b.requirement_type)
      + '</table></div>'
      + ((b.platforms?.length||b.num_influencers) ? '<div class="section"><h2>Influencer Marketing</h2><table>'
        + row('Platforms', b.platforms?.join(', ')) + row('Influencer Tier', b.influencer_tier?.join(', '))
        + row('No. of Influencers', b.num_influencers) + row('Content Type', b.content_type)
        + row('Genre', b.genre?.join(', ')) + row('Deliverables', b.deliverables)
        + row('Campaign Timeline', b.campaign_timeline) + row('Content Format', b.content_format?.join(', '))
        + row('Client Brief', b.client_brief) + '</table></div>' : '')
      + ((b.business_objective?.length) ? '<div class="section"><h2>360° Marketing</h2><table>'
        + row('Business Objective', b.business_objective?.join(', ')) + row('Channels Required', b.channels_required?.join(', '))
        + row('Timeline', b.timeline_360) + row('Deliverables', b.deliverables_360)
        + row('Client Brief', b.brief_360) + '</table></div>' : '')
      + ((b.primary_goal?.length) ? '<div class="section"><h2>Performance Marketing</h2><table>'
        + row('Primary Goal', b.primary_goal?.join(', ')) + row('Platforms', b.platforms_perf?.join(', '))
        + row('Monthly Ad Spend', b.ad_spend_budget) + row('Pixel Status', b.pixel_status)
        + row('Timeline', b.timeline_perf) + row('Client Brief', b.brief_perf) + '</table></div>' : '')
      + ((b.client_website||b.total_budget) ? '<div class="section"><h2>Client Assets & Budget</h2><table>'
        + row('Website', b.client_website) + row('Social Handles', b.social_handles)
        + row('Product Link', b.product_link) + row('Reference Campaigns', b.reference_campaigns)
        + row('Total Budget', b.total_budget) + row('Budget Allocation', b.budget_allocation) + '</table></div>' : '')
      + ((b.website_platform||b.target_keywords) ? '<div class="section"><h2>SEO / Website</h2><table>'
        + row('Platform', b.website_platform) + row('Primary Objective', b.primary_objective)
        + row('Target Keywords', b.target_keywords) + row('Competitors', b.competitors)
        + row('Monthly Traffic', b.monthly_traffic) + row('SEO Scope', b.seo_scope)
        + row('Blog Strategy', b.blog_strategy) + row('Website Optimization', b.website_optimization)
        + row('Timeline', b.timeline_seo) + '</table></div>' : '')
